...
\note{Science for Life Laboratory is a national center}
...
\note{NGI is an sequencing infrastructure}
...
\note{NBIS is a Bioinformatics support infrastructure, and as you can see they are located all over Sweden, except for the North which is pretty much empty}
...
\note{But in the North there is this National park called Sarek}
...
\note{This park is pretty wild and inaccessible, so it's like...}
...
\note{...Mordor, and we found that such a landscape was a perfect metaphor for the genomic landscape we wanted to explore.}
...
\note{Sarek is a pipeline powered by Nextflow, developed at NGI with NBIS and BTB support}
...
\note{Nextflow is a dataflow language

where explicitly defined inputs and outputs connect operations

-> inherently parallel and can work well in large, decentralized systems.

Helps in generating portable pipelines

And also Shareable and reproducible pipelines with the help of containers
}
...
\note{Singularity is a container engine

Specific for HPC environnment
...
\note{
Sarek germline for analyzing normal samples and

Sarek somatic for analyzing tumor/normal pairs (with optionnal relapses)
}
...
\note{\begin{itemize}
\item Reads mapped to reference with bwa
\begin{itemize}
	\item To provide a common coordinate framework
\end{itemize}
\item Duplicates marked with picard MarkDuplicates
\begin{itemize}
	\item To identify read pairs duplicated from artifactual processes
\end{itemize}
\item Realign indels with GATK IndelRealigner
\begin{itemize}
	\item To facilitate somatic calling for T/N pair
\end{itemize}
\item Recalibrate with GATK BaseRecalibrator
\begin{itemize}
	\item To detect and correct errors in the base quality scores
\end{itemize}
\end{itemize}}
...
\note{Variant Calling tools for Germline analysis: HaplotypeCaller and Strelka2 for SNVs and small indels
...
\note{Variant Calling tools for Somatic analysis: MuTect2 (we're removing Mutect1), Freebayes and Strelka2 for SNVs and small indels

Manta for Structural Variants

ASCAT, and adding Control-FREEC for CNVs}
...
\note{Here we are comparing the number and overlap of Somatic SNV calls for a WGS public medulloblastoma dataset

We can see that MuTect1 does not add many true calls, so it was a good reason to finally remove it
}
...
\note{Annotation is done with VEP and SnpEff, and we're also working on Prioritization}
...
\note{For prioritization we're feeding SnpEff output to VEP and analyzing the resulting annotated VCFs}
...
\note{We use also several QC tools and all reports are gathered within MultiQC into a single final QC report}
...
\note{Just to conclude about what's under the hood}
...
\note{Preprocessing is done with the whole genome and Variant calling only on the targeted regions}
...
\note{Obviously GRCh37 and GRCh38, and it's possible to add your own custom genome of reference}
...
\note{Samples from the Swedish Childhood Tumor Biobank (article in preparation)
The whole SweGen dataset
Clinical samples within pilot project with Genomic Medicine Sweden initiative
}
...
\note{nf-core is an international collaboration to develop, share and use Nextflow pipeline}
...
\note{Thanks to everyone involved within Sarek and all my collaborators}
